...
struct NewRequestPaidPopUpView: View {
    ...
    init(showSheet: Binding<Bool>, voyage: VoyageSession, dependencies: AppDependencies = .live) {
        ...
        _viewModel = StateObject(wrappedValue: NewRequestPopUpViewModel(
            paymentRepository: LivePaymentRepository(network: dependencies.networkRepository),
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 10) {
                    ...
                    if let sheet = stripeSheet {
                        PaymentSheetWrapper(paymentSheet: sheet) { result in
                            ...
                            case .completed:
                                ...
                                isShowToast = true
                                viewModel.handleVoyagerStripePaymentCompleted(
                                    voyageId: voyageId,
                ...
                NavigationLink(
                    destination: TrackRidePopupView(
                ...
                NavigationLink(
                    destination: PaymentPopUpView(type: .VoyagerPayment, receiptEmail: viewModel.receiptEmailSnippet),
            ...
            .navigationBarBackButtonHidden(true)
            .onAppear {
                displayDate = viewModel.bookingDisplayLabel(from: voyage.bookingDateTime)
            }
        }
    }
}
