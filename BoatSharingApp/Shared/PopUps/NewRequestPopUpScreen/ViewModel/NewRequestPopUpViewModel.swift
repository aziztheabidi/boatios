...
final class NewRequestPopUpViewModel: ObservableObject {
    ...
    init(paymentRepository: PaymentRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    func bookingDisplayLabel(from rawDate: String) -> String {
    ...
    func handleVoyagerStripePaymentCompleted(
        voyageId: String,
        paymentIntentId: String,
        onDone: @escaping () -> Void
    ) {
        Task { @MainActor in
            await completeVoyagerPaymentAfterDelay(voyageId: voyageId, paymentIntentId: paymentIntentId)
            onDone()
        }
    }
    ...
    private func fetchPaymentIds(voyagerId: String) {
        ...
        Task { @MainActor in
            do {
                let response = try await paymentRepository.initiateVoyagePayment(voyagerId: voyagerId)
                mutate {
    ...
    private func confirmVoyagerPayment(voyageId: String, paymentIntentId: String) {
        ...
        Task { @MainActor in
            do {
                _ = try await paymentRepository.confirmVoyagePayment(voyageId: voyageId, paymentIntentId: paymentIntentId)
                mutate {
    ...
    private func confirmSponsorPayment(voyageId: String, paymentIntentId: String) {
        ...
        Task { @MainActor in
            do {
                _ = try await paymentRepository.confirmSponsorPayment(voyageId: voyageId, paymentIntentId: paymentIntentId)
                mutate {
                    $0.isPaymentConfirmedLoaded = false
                    $0.paymentConfirmed = true
                }
    ...
    private func fetchSponsorPaymentIds(voyagerId: String, sponsorId: String, user: String) {
        ...
        Task { @MainActor in
            do {
                let response = try await paymentRepository.initiateSponsorPayment(parameters: parameters)
                mutate {
                    $0.isPaymentLoaded = false
                    $0.initialPaymentSuccess = true
                    $0.paymentData = response.obj
