...
final class BusinessStepOneViewModel: ObservableObject {
    ...
    private func performRegisterUser(
        ...
        Task {
            do {
                ...
                switch userType {
                case .voyager:
                    response = try await networkRepository.saveVoyagerProfile(parameters: parameters)
                case .business:
                    response = try await networkRepository.saveBusinessProfile(parameters: parameters)
                }
    ...
    private func fetchBusinessProfile(userId: String) {
        ...
        Task {
            do {
                let response = try await networkRepository.getBusinessProfile(userId: userId)
    ...
    private func fetchVoyagerProfile(userId: String) {
        ...
        Task {
            do {
                let response = try await networkRepository.getVoyagerProfile(userId: userId)
                self.isBusniesProfileLoading = false
                self.BusinessProfile = response.obj
            } catch {
                self.isBusniesProfileLoading = false
