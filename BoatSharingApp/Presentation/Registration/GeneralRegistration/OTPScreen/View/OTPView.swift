...
struct OTPView: View {
    ...
    init(email: String, dependencies: AppDependencies = .live) {
        self.email = email
        _viewModel = StateObject(wrappedValue: OTPViewModel(networkRepository: LiveRegistrationRepository(network: dependencies.networkRepository), tokenStore: dependencies.tokenStore))
    }
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                NavigationLink(destination: CreatePasswordView(), isActive: $viewModel.shouldNavigate) {
                    EmptyView()
