...
final class CaptainRegStepTwoViewModel: ObservableObject {
    ...
    init(networkRepository: RegistrationRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    func CaptainDocument(UserId: String, LicenseNumber: String, LicenseExpiration: String, TypeOfLicense: String, InsuranceCompany: String, PolicyNumber: String, PolicyExpiration: String) {
        ...
        Task {
            do {
                let response = try await networkRepository.saveCaptainDocument(parameters: parameters)
    ...
    func getCaptainDocument() {
        ...
        Task {
            do {
                let response = try await networkRepository.getCaptainDocument(userId: userId)
                self.isLoading = false
