...
final class RoleSelectionViewModel: ObservableObject {
    ...
    private let networkRepository: AccountRepositoryProtocol
    private let sessionPreferences: SessionPreferenceStoring

    init(networkRepository: AccountRepositoryProtocol, sessionManager: SessionManaging, sessionPreferences: SessionPreferenceStoring) {
    ...
    private func performUpdateRole(userId: String, role: String) {
        ...
        Task { @MainActor in
            do {
                let response = try await networkRepository.updateRole(parameters: parameters)
