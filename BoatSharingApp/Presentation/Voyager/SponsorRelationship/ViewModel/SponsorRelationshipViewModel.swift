...
final class SponsorRelationshipViewModel: ObservableObject {
    ...
    private let networkRepository: RelationshipRepositoryProtocol
    private let sessionPreferences: SessionPreferenceStoring

    init(networkRepository: RelationshipRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    private func fetchRelationship() {
        Task {
            do {
                let response = try await networkRepository.getVoyagerRelationship()
    ...
    private func performFollow(voyagerId: String) {
        ...
        Task {
            do {
                _ = try await networkRepository.followVoyager(parameters: ["VoyagerUserId": voyagerId])
    ...
    private func performUnfollow(voyagerId: String) {
        ...
        Task {
            do {
                _ = try await networkRepository.unfollowVoyager(parameters: ["VoyagerUserId": voyagerId])
                self.isLoading = false
                self.fetchRelationship()
            } catch {
                self.isLoading = false
