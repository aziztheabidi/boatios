...
final class AddSponsorsViewModel: ObservableObject {
    ...
    init(networkRepository: DashboardRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    private func fetchFollowedVoyagers() {
        ...
        Task {
            do {
                let response = try await networkRepository.getFollowedVoyagers(userId: userId)
