...
final class PastVoyageViewModel: ObservableObject {
    ...
    private let networkRepository: DashboardRepositoryProtocol
    ...
    init(networkRepository: DashboardRepositoryProtocol, identityProvider: SessionPreferenceStoring, initialRole: String) {
    ...
    private func fetchVoyagerPastVoyages(userId: String) {
        ...
        Task {
            do {
                let response = try await networkRepository.getVoyagerPastVoyages(userId: userId)
    ...
    private func fetchCaptainPastVoyages() {
        ...
        Task {
            do {
                let response = try await networkRepository.getCaptainPastVoyages()
                self.isPastVoyageLoading = false
                if response.voyages.isEmpty {
                    self.errorMessage = "No past voyages found."
