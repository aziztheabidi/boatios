...
final class VoyagerHomeViewModel: ObservableObject {
    ...
    init(networkRepository: DashboardRepositoryProtocol, identityProvider: SessionPreferenceStoring) {
    ...
    private func fetchActiveDocks() {
        ...
        Task {
            do {
                let docks = try await networkRepository.getActiveDocks()
    ...
    private func fetchActiveVoyage(userid: String) {
        ...
        Task {
            do {
                let session = try await networkRepository.getVoyagerActiveVoyage(voyagerUserId: userid)
