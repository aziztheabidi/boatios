...
final class FutureVoyageViewModel: ObservableObject {
    ...
    private let networkRepository: TravelRepositoryProtocol
    ...
    init(networkRepository: TravelRepositoryProtocol, identityProvider: SessionPreferenceStoring) {
        self.networkRepository = networkRepository
        self.identityProvider = identityProvider
        self.sponsorPaymentRequestViewModel = NewRequestPopUpViewModel(
            paymentRepository: LivePaymentRepository(network: networkRepository),
    ...
    private func fetchFutureVoyages(userid: String) {
        ...
        Task {
            do {
                let response = try await networkRepository.getFutureBookedVoyages(userId: userid)
    ...
    private func performVoyageCancellation(voyageId: String) {
        ...
        Task {
            do {
                _ = try await networkRepository.cancelVoyage(voyageId: voyageId)
    ...
    private func performVoyageConfirmation(voyageId: String) {
        ...
        Task {
            do {
                let response: VoyageConfirmationResponse = try await networkRepository.confirmVoyage(voyageId: voyageId)
                self.loadingVoyageId = nil
    ...
    private func performSponsorPaymentConfirmation(voyageId: String, paymentIntentId: String) {
        Task {
            do {
                _ = try await networkRepository.confirmSponsorPayment(voyageId: voyageId, paymentIntentId: paymentIntentId)
