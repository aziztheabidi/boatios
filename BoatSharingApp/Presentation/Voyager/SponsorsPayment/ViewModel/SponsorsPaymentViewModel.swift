...
final class SponsorsPaymentViewModel: ObservableObject {
    ...
    private let networkRepository: PaymentRepositoryProtocol
    private let sessionPreferences: SessionPreferenceStoring
    private var hasLoaded = false
    private var navigateToSuccessTask: Task<Void, Never>?

    init(networkRepository: PaymentRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    private func fetchSponsorPayments() {
        ...
        Task {
            do {
                let response = try await networkRepository.getSponsorPayments(userId: userId)
