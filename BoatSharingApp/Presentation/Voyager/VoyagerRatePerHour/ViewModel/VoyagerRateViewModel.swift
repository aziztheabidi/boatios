...
final class VoyagerRateViewModel: ObservableObject {
    ...
    private let networkRepository: BookingRepositoryProtocol
    private let sessionPreferences: SessionPreferenceStoring

    init(networkRepository: BookingRepositoryProtocol, sessionPreferences: SessionPreferenceStoring) {
    ...
    func getVoyagerRate(using draft: VoyageDraft) {
        ...
        Task {
            do {
                let response = try await networkRepository.calculateFare(
    ...
    func findBoat(
        ...
        Task {
            do {
                _ = try await networkRepository.findBoat(parameters: parameters)
    ...
    func bookVoyage(
        ...
        Task {
            do {
                let response = try await networkRepository.bookVoyage(parameters: parameters)
