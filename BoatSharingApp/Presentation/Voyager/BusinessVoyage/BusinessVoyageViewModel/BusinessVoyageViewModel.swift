...
final class BusinessVoyageViewModel: ObservableObject {
    ...
    init(networkRepository: RelationshipRepositoryProtocol) {
    ...
    private func fetchBusinessRelationship() {
        ...
        Task {
            do {
                let response = try await networkRepository.getBusinessRelationship()
    ...
    private func performFollowBusiness(id: Int) {
        ...
        Task {
            do {
                let response = try await networkRepository.followBusiness(businessDockId: id)
                self.isLoading = false
    ...
    private func performUnfollowBusiness(id: Int) {
        ...
        Task {
            do {
                let response = try await networkRepository.unfollowBusiness(businessDockId: id)
                self.isLoading = false
                if response.status.isSuccess {
                    self.successMessage = response.message
                    self.fetchBusinessRelationship()
